...
import { WORKOUT_PROGRAM, Exercise } from "../lib/workout-data";
import {
  getLevelProgress,
  getRankForLevel,
  formatNumber,
...
function DashboardInner({ userId, initialData }: { userId: string; initialData: InitialData }) {
  ...
  // Strict Workout Tracker State
  const [activeSessionId, setActiveSessionId] = useState<string | null>(null);
  const [lastCompletedDates, setLastCompletedDates] = useState<Record<string, string>>({});
  const [activeExerciseInfo, setActiveExerciseInfo] = useState<Exercise | null>(null);

  // Load completed dates
  useEffect(() => {
    try {
      const dates = JSON.parse(localStorage.getItem('workout-completions') || '{}');
  ...
  const handleCompleteTask = useCallback((taskId: string) => {
    // Legacy complete task (keep for challenges if needed)
  }, [challenges, totalXp, userId]);

  const handleFinishWorkout = (xpEarned: number) => {
    setActiveSessionId(null);
    setTaskCompleteAnim({ visible: true, xp: xpEarned });
    ...
    setTimeout(() => {
      ...
      startTransition(async () => {
        try {
          const result = await serverAwardXp(userId, xpEarned, "todo"); // Temporary type matching
          setTotalXp(result.totalXp);
          setStreakDays(result.streakDays);
        } catch (e) { console.error("Could not sync workflow", e) }
      });
    }, 400);
  };
    ...
    <div className="min-h-screen bg-gray-1 text-gray-12 flex flex-col w-full relative">
      ...
      <main className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 scroll-smooth">
        {viewingProgram && activeChallenge ? (
          ...
          <ActiveWorkoutScreen
            session={WORKOUT_PROGRAM.find(s => s.id === activeSessionId)!}
            onBack={() => setActiveSessionId(null)}
            onFinishWorkout={handleFinishWorkout}
            onOpenInfo={(ex) => setActiveExerciseInfo(ex)}
          />
        ) : activeTab === "home" ? (
          <ProgramListScreen
      ...
      />
      <InfoModal exercise={activeExerciseInfo} onClose={() => setActiveExerciseInfo(null)} />
